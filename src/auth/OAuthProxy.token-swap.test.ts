...
/**
 * Tests for swap mode refresh token handling.
 * When enableTokenSwap is true, clients send FastMCP JWT refresh tokens.
 * The proxy must verify the JWT, look up upstream tokens, refresh with provider,
 * and issue new FastMCP tokens (with one-time use enforcement).
 */
describe("OAuthProxy - Swap Mode Refresh Token", () => {
  ...
  beforeEach(() => {
    // Default mock for upstream token refresh
  ...
  /**
   * Helper to perform initial token exchange and get initial FastMCP tokens
   */
